...
For multi-disc games, put all your CHDs in the same folder and create a .m3u playlist file. This command will do it for you and name it the same thing the directory is named: `find . -maxdepth 1 -type f -name "*.chd" -exec basename {} \; | sort -Vr > "$(basename "$(pwd)").m3u"`
